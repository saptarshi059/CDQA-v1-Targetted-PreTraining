...
pd.DataFrame(zip(selected_ents_text_dict.keys(), texts), columns = ['ent', 'text']).to_csv(f'mini_corpus-{no_of_ents_to_select}T{no_of_results_per_entity}CpT.csv', index=False)